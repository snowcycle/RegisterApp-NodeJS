...
/*
export const queryByEmployeeId = async (employeeId?: number): Promise<CommandResponse<Employee>> => {
	if (Helper.isBlankNumber(employeeId)) {
	...
	return EmployeeRepository.queryByEmployeeId(<number>employeeId)
		.then((queriedEmployee: (EmployeeModel | null)): Promise<CommandResponse<Employee>> => {
			...
			return Promise.resolve(<CommandResponse<Employee>>{
				status: 200,
				data: EmployeeHelper.mapProductData(queriedEmployee)
			});
		});
};
*/
